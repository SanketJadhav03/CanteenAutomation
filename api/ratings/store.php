...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($order_id) || empty($product_id) || empty($customer_id) || empty($rating) || empty($rating)) {
        echo json_encode([
            'status' => 'error',
    ...
    if ($stmt = $conn->prepare($sql)) {
        $stmt->bind_param('iiii', $order_id, $product_id, $customer_id, $rating, $review_rating);
        if ($stmt->execute()) {
            echo json_encode([
                'status' => 'success',
